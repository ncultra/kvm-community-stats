...
# try to match a regex in options.email to line

    if options.regex_email:
        for x in options.regex_email:
            if re.search(x, line):
                return True
        return False
...
def setup_directories_list():
    global directories_list
    
    fp = file(options.directory_file)
    while True:
        line = fp.readline()
        if not line:
            break;
        ...
        if not line:
            continue
# "--" is the flag that preceds a directory argument to "git log"
        line = " -- " + line
        directories_list.append(line)
    fp.close
...
def print_commits(commits):
    ...
    for commit in commits:
        ...
        if options.one_line==True:
            continue

        if (not should_lc) & (not options.one_line):
            print >> output, "%s" % str1
            continue

        str2 = " (+%d/-%d)" % (commit[1], commit[2])
        if not options.one_line:
...
parser.add_option("-E", "--email-file",
                  ...
                  default=None)
parser.add_option("-D", "--directory-file",
                  help="File with directories for which to collect statistics",
                  default=None)
...
parser.add_option("-u", "--until", help="--until flag for git log", default="")
...
parser.add_option("-c", "--commit-text", help="Print the full commit text",
                  default=False, action="store_true")
parser.add_option("-L", "--one-line", help="print one line for each author",  
                  default=False, action="store_true")
parser.add_option("-O", "--stdout", help="print output to stdout (terminal) without paging", 
                  default=False, action="store_true")
parser.add_option("-R", "--regex-email", help="Regular expressions denoting email addresses of interest",
                  default=[], action="append")
...
if options.directory_file:
    setup_directories_list()
#    sys.stderr.write("%s\n" % directories_list)
...
if not options.pipe:
    ...
    if options.until:
        git_args += " --until=\"" + options.until + "\""
# for at least some versions of git --log the "-- <dir>" argument(s) must be last"
    if options.directory_file:
        git_args += " ".join(directories_list)
...
if (not options.diffstat) & (not options.one_line):
